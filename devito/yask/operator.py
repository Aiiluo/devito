...
def find_offloadable_trees(iet):
    ...
    *and* all of the grids accessed by the enclosed equations are homogeneous
    ...
    for tree in retrieve_iteration_tree(iet):
        ...
        inner_iteration = tree[-1]
        if not all(i.is_Expression for i in inner_iteration.nodes):
            # Wouldn't know how to offload this IET to YASK
            continue
        if not all(i.affine_dimensions() for i in inner_iteration.nodes):
        ...
        if not all(i.dim in grid.dimensions for i in grid_iterations):
            continue
